...
public class MockServiceTest {
    ...
    public void shouldAtLeastTwoTimesForChampionGetName() {
        Champion champions = mock(Champion.class);
        champions.setName("람머스");
        champions.setPosition("정글");
        champions.setHasSkinCount(3);

        System.out.println(champions.getName());
        System.out.println(champions.getName());
        verify(champions, times(2)).getName();
    ...
    public void shouldAtMostThreeTimesForChampionGetName() {
        Champion champions = mock(Champion.class);
        champions.setName("리산드라");
        champions.setPosition("미드");
        champions.setHasSkinCount(3);

        System.out.println(champions.getName());
        System.out.println(champions.getName());
        System.out.println(champions.getName());
        verify(champions, times(3)).getName();
    ...
    public void shouldPassWhenNeverRunChampionSetName() {
        Champion champions = mock(Champion.class);
//      champions.setName("리신");
        champions.setPosition("탑");
        champions.setHasSkinCount(3);

        verify(champions, never()).setName(anyString());
    ...
    public void shouldOneTimeRunForChampionGetName() {
        Champion champions = mock(Champion.class);
        champions.setName("케이틀린");
        champions.setPosition("미드");
        champions.setHasSkinCount(3);

        verify(champions, timeout(200).atLeastOnce()).setName(anyString());
    }
    ...
    public void List객체사이즈검증테스트() {
        ...
        champions.add(new Champion("루시안", "미드", 6));
        champions.add(new Champion("세나", "서포터", 3));
        champions.add(new Champion("모르가나", "정글", 7));
    ...
    @Test
    public void 가짜챔피언객체호출확인() {
        when(mockService.findByName(anyString())).thenReturn((new Champion("리산드라", "미드", 6)));
        assertThat(mockService.findByName("세주아니").getHasSkinCount(), is(6));
    }

    // 4. 2개 이상의 가짜 챔피언 객체를 List로 만들어 리턴하고, 하나씩 해당 객체를 검색한 뒤 검색을 위해 호출한 횟수를 검증하세요.
    @Test
    public void List가짜객체호출회수검증테스트() {
        List<Champion> champions = new ArrayList<>();
        champions.add(new Champion("리산드라", "미드", 6));
        champions.add(new Champion("다리우스", "탑", 8));
